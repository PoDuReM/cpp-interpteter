...
Function : type name '(' ')' '{' Body '}'    {Function0 (getType $1) $2 $6}
...
Body : Statement Body { $1 : $2 }
     | {- empty -}    { [] }

Statement : type name '=' Expr ';' { VarDecl (getType $1) $2 $4 }
          | type name ';'          { VarDecl (getType $1) $2 (defaultValue $ getType $1) }
          | name '=' Expr  ';'     { VarAssi $1 $3 }
          | if '(' Expr ')' '{' Body '}' else '{' Body '}'  { IfExpr $3 $6 $10 }
          | if '(' Expr ')' '{' Body '}'  {IfExpr $3 $6 []}
          | while Expr '{' Body '}' {WhileExpr $2 $4}
          | cout '<<' Expr ';'          {Cout $3}
          | cin '>>' name ';'       { Cin $3 }
          | return Expr ';'         { VarAssi "funcRetVal'" $2}
          | name '(' ')' ';'   {FuncSCall0 $1}
          | name '(' Expr ')' ';' {FuncSCall1 $1 $3}
          | name '(' Expr ',' Expr ')' ';' {FuncSCall2 $1 $3 $5}

Expr  : Expr '<' Expr2  { Le $1 $3 }
      | Expr '>' Expr2  { Gt $1 $3 }
      | Expr '==' Expr2 { Eq $1 $3 }
      | Expr '<=' Expr2 { Leq $1 $3}
      | Expr '>=' Expr2 { Geq $1 $3}
...
Expr2 : Expr2 '+' Expr3 { Add $1 $3 }
      | Expr2 '-' Expr3 { Sub $1 $3 }
      | Expr3            { $1 }
        

Expr3 : Expr3 '*' Expr4 { Mul $1 $3 }
      | Expr3 '/' Expr4 { Div $1 $3 }
      | Expr4            { $1 }
