...
toDslFunction2 mp (Function2 typ1 _ typ2 name2 typ3 name3 body) = 
  sFun2 typ1 (\arg1' arg2' retFunc' -> 
    let newFuncMap = fromList [("funcRetVal'", retFunc'), (name2, arg1'), (name3, arg2')] in 
...
toDslBody mpF mp (x:xs) = 
  ...
  case x of 
    (VarDecl typ name expr) -> 
      ...
      else sWithVar typ (toDslExpr' expr) (\var -> toDslBody' (insert name var mp) xs)
    (VarAssi name expr) ->
      if member name mp
      then (mp ! name) @= toDslExpr' expr # toDslBody' mp xs
      else error $ "variable " <> name <> " does not exist"
    (IfExpr a b c) -> sIf (toDslExpr' a) (toDslBody' mp b) (toDslBody' mp c) # toDslBody' mp xs
    (WhileExpr a b) -> sWhile (toDslExpr' a) (toDslBody' mp b) # toDslBody' mp xs
    ...
    (FuncSCall0 name) -> if member name (arg0 mpF)
                         ...
                         else error $ "function " <> name <> " with 0 arguments does not exists"
    ...
    (FuncSCall2 name expr1 expr2) -> 
      ...
      then sCallFunc (((arg2 mpF) ! name) (toDslExpr' expr1) (toDslExpr' expr2)) # toDslBody' mp xs
      else error $ "function " <> name <> " with 1 arguments does not exists"
toDslBody _ _ [] = empt
